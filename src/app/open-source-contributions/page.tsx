...
interface ApiResponse {
  ...
  pushed_at?: string | null; stargazers_count?: number | null;
  html_url: string; owner: { login: string };
}
interface Repository {
  id: number; name: string; description: string | null;
  pushed_at: string; stargazers_count: number; html_url: string;
  owner: { login: string };
}
const transform = (r: ApiResponse): Repository => ({
  id: r.id, name: r.name,
  description: r.description ?? null,
  pushed_at: r.pushed_at ?? new Date().toISOString(),
  stargazers_count: r.stargazers_count ?? 0,
  html_url: r.html_url, owner: { login: r.owner.login }
});
const isApiResponse = (x: unknown): x is ApiResponse =>
  typeof x === "object" && x !== null && typeof (x as any).id === "number";
...
export default function OpenSource() {
  ...
  useEffect(() => {
    ...
    async function fetchAllEvents(user: string): Promise<RepoEvent[]> {
      ...
      while (more) {
        const res = await octokit.rest.activity.listPublicEventsForUser({
          username: user, per_page: 100, page: pg
        });
    ...
    async function gather() {
      try {
        setLoading(true);
        // own repos
        const own = await octokit.rest.repos.listForUser({ username: "Shubham-Sahoo", per_page: 100 });
        const ownRepos = own.data.filter(isApiResponse).map(transform).slice(0, 20);

        // contributed repos
        const events = await fetchAllEvents("Shubham-Sahoo");
        const contribDetails = await Promise.all(
          events
            .filter(e => ["PushEvent", "PullRequestEvent", "PullRequestReviewCommentEvent"].includes(e.type || ""))
            .map(async e => {
        ...
        const contribRepos = contribDetails.filter(isApiResponse).map(transform);

        // merge & deduplicate by stars
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 py-12 px-6">
      <Head><title>Shubham Sahoo – Open-Source</title></Head>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-white mb-4">Shubham Sahoo</h1>
          <p className="text-xl text-gray-300">AI Researcher | Open Source Enthusiast | Deep Learning Engineer</p>
        </div>
        ...
        <div className="bg-gray-800 bg-opacity-50 rounded-lg p-8">
          <h2 className="text-3xl font-bold text-white mb-8 text-center">Recent GitHub Repositories</h2>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {view.map(r => (
                  <div key={r.id} className="bg-gray-700 bg-opacity-50 rounded-lg p-6 hover:bg-opacity-70 transition-all">
                    <h3 className="text-xl font-semibold text-white mb-3">{r.name}</h3>
                    <p className="text-gray-300 mb-4 text-sm">{r.description || "No description provided."}</p>
